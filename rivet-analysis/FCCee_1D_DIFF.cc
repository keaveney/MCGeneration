...
namespace Rivet {
  ...
  class FCCee_1D_DIFF : public Analysis {
  private:

  public:

    /// Constructor
    DEFAULT_RIVET_ANALYSIS_CTOR(FCCee_1D_DIFF);
    ...
    void analyze(const Event& event) {
	  ...
	  //	  std::cout<<"Top cand kinematics =   "<<  _b1P.pT() <<"  "<<  _b2P.pT()<<" "<<   _lepton1P.pT()<<" "<< _lepton2P.pT() <<std::endl;
	  ...
	  //final event cuts
	  if (llLVec.mass() < 20.0) return;
	  
	  //           std::cout<<" EVENT PASSED< FILLING HISTOS "<<std::endl;
	  //fill histos 
	  //std::cout <<"vars  = "<< t1LVec.pT()<<" " <<	 t2LVec.pT() << " " <<_tops[0].pT()<<" "<< _tops[1].pT()<<" "<<t1_ttrf.pT()<<" "<<t1LVec.rapidity()<<" "<< t2LVec.rapidity()<<" "<< _tops[0].rapidity()<<" " <<_tops[1].rapidity()<<" "	 <<ttLVec.pT()<< ttLVec.rapidity()<< " "<<ttLVec.mass() <<" "<< tt_deltaphi << " "<< tt_deltarap << " " << w1dau1LVec.pT()<< " "<< w2dau1LVec.pT() << " " << w1dau1LVec.pseudorapidity()<< " "<< w2dau1LVec.pseudorapidity()<<" " << _pleptons[0].pT()<<" "<<_pleptons[1].pT()<<" "<< _pleptons[0].pseudorapidity()<< " "<<_pleptons[1].pseudorapidity()<<" "<< llLVec.pT()<<" "<<   llLVec.mass() << " "<< ll_deltaphi << " " << ll_deltaeta <<" "<<_bottoms[0].pT()<<" "<<_bottoms[1].pT()<< " " << _bottoms[0].rapidity()<<" " << _bottoms[1].rapidity()<<" "<<bbLVec.mass()<<" " <<bbLVec.pT()<<" "<<_jets.size()<< std::endl;
	  ...
	  _h_l_pT->fill(w2dau1LVec.pT(), weight);
	  _h_l_eta->fill(w1dau1LVec.pseudorapidity(), weight);
    ...
    void finalize() {
      // normalize(_h_YYYY); // normalize to unity
           ...
           std::cout<<"SCALIN  HISTOS "<<std::endl;
   ...
   struct GreaterByPt
   {
    ...
    double _lepR = 0.1, _lepMinPt = 20, _lepMaxEta = 2.4;
    double _jetR = 0.4 , _jetMinPt = 30 , _jetMaxEta = 2.4;
    ...
    Histo1DPtr _h_l_pT, _h_lbar_pT, _h_l_eta,  _h_lbar_eta,  _h_llead_pT,  _h_ltrail_pT,  _h_llead_eta,  _h_ltrail_eta,  _h_llbar_pT,  _h_llbar_mass,   _h_llbar_delphi,  _h_llbar_deleta,  _h_blead_pT,   _h_btrail_pT,  _h_blead_eta,   _h_btrail_eta,  _h_bbbar_mass,   _h_bbbar_pT,  _h_njet ;
  ...
  DECLARE_RIVET_PLUGIN(FCCee_1D_DIFF);
