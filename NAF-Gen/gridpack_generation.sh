...
#Need later version of MG for NLO EFT model
...
echo "Searching for gridpack folder in "  ${AFS_GEN_FOLDER}/${name}_gridpack

if [ ! -d ${AFS_GEN_FOLDER}/${name}_gridpack ]; then
  #directory doesn't exist, create it and set up environment

    echo "Grid pack directory doesn't already exist :) Creating it and setting up environement..."
...
#   export RELEASE=CMSSW_5_3_30
  ...
  #############################################
#Dont apply patches for 2_5_2

#  patch -l -p0 -i $PRODHOME/patches/mgfixes.patch
#  patch -l -p0 -i $PRODHOME/patches/models.patch

  # agrohsje add model directories 
  MGDIR=${MG%.tar.gz} 
  MGDIR=${MGDIR//./_} 
  cp -rp ${PRODHOME}/EFT_tt/models/${name} ${MGDIR}/models/.
...
#       echo "set cluster_local_path `${LHAPDFCONFIG} --datadir`" >> mgconfigscript 
  ...
  echo "Running ./bin/mg5_aMC mgconfigscript  in dir" $PWD
  ...
  echo   "Copying cards to current diectory!"
...
echo "Recompiling CutTools and IREGI.. in directory " $PWD

####RECOMPILE CutTools and IREGI
  cd ../MG5_aMC_v2_5_2/vendor/CutTools
  make clean
  make
